...
func CreateComponent(index string, name string, city string, historyConfig json.RawMessage, mapFilter json.RawMessage, timeFrom string, timeTo *string, updateFreq *int64, updateFreqUnit string, source string, shortDesc string, longDesc string, useCase string, links pq.StringArray, contributors pq.StringArray, queryType string, chartConfig ComponentChart) (cityComponent CityComponent, err error) {
	component := Component{
		Index: index,
		Name:  name,
	}
	...
	queryCharts := QueryCharts{
		...
		UpdatedAt:      time.Now(),
		QueryType:      queryType,
		QueryChart:     "SELECT x_axis, y_axis, icon, name, type, data FROM (VALUES (NULL::text, NULL::text, NULL::text, NULL::text, NULL::text, NULL::numeric)) _(x_axis, y_axis, icon, name, type, data) WHERE x_axis IS NOT NULL",
	}
	...
	err = DBManager.Table("components").Where("index = ?", index).FirstOrCreate(&component).Error
	if err != nil {
	...
	err = DBManager.Table("component_charts").Where("index = ?", index).FirstOrCreate(&chartConfig).Error
	if err != nil {
		return cityComponent, err
	}

	cityComponent.ID = component.ID

	var existingQueryChart QueryCharts
	err = DBManager.Table("query_charts").Where("index = ? AND city = ?", index, city).First(&existingQueryChart).Error
