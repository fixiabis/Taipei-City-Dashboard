...
async function handleSubmit() {
  ...
  if (
    ...
    dialogStore.showNotification("fail", "新建失敗，請確實填寫所有必填欄位");
    return;
  }
  try {
    const createRes = await http.post(`/component/`, params.value);
    const getRes = await http.get(
      `/component/${createRes.data.data.id}?city=${city}`
    );
    ...
    <div class="admincomponenttemplate">
      ...
      <div class="admincomponenttemplate-content">
        <div class="admincomponenttemplate-settings">
          ...
          >
            <label>City*</label>
            <select v-model="params.city" required>
            ...
            <div class="two-block">
              <input v-model="params.index" type="text" required />
            </div>
            <label>資料來源*</label>
            <input
              ...
              @keypress.enter="
                () => {
                  if (tempInputStorage.link.length > 0) {
                    params.links.push(tempInputStorage.link);
              ...
              @keypress.enter="
                () => {
                  if (tempInputStorage.contributor.length > 0) {
                    params.contributors.push(tempInputStorage.contributor);
          ...
          >
            <label>圖表資料型態</label>
            <select v-model="params.query_type" required>
            ...
            <label>資料單位*</label>
            <input
              v-model="params.chart_config.unit"
              type="text"
              :minlength="1"
              :maxlength="6"
              required
            ...
            <SelectButtons
              :tags="
                chartsPerDataType[params.query_type]
              "
              :selected="params.chart_config.types"
              :limit="3"
              @updatetagorder="
                (updatedTags) => {
                  params.chart_config.types =
                    updatedTags;
                }
              "
            />
            <label>圖表顏色</label>
            <InputTags
              :tags="params.chart_config.color"
              :color-data="true"
              @deletetag="
                (index) => {
                  params.chart_config.color.splice(
                    index,
                    1
                  );
                }
            ...
            <input
              ...
              type="color"
              class="admincomponenttemplate-settings-inputcolor"
              @focusout="
                () => {
                  if (
                    tempInputStorage.chartColor.length === 7
                  ) {
                    params.chart_config.color.push(
